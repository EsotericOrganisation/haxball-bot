...
```json
{
	...
	"panel": {
		"bots": [
			{
				"name": "example1",
				"displayName": "Example room 1",
				"path": "path/to/example1.js"
			},
			{
				"name": "example2",
				"displayName": "Example room 2",
				"path": "path/to/example2.js"
			}
		],
		"discordToken": "a discord bot token",
		"clientId": "your bot's user id",
		"channels": ["list", "of", "channels", "where", "commands", "can", "be", "used"]
	}
}
```

Put the module in your directory and open the server with a simple command:

```bash
node haxball-server open -f config.json
```

Using Linux? Lacking a UI?
...
```json
{
	...
	"panel": {
		"bots": [
			...
			{
				"name": "classic",
				"displayName": "Classic room",
				"path": "./bots/classic.js"
			}
		],
